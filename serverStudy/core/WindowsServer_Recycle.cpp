...
namespace recycle
{
	...
	PostAcceptRecycle* PostAcceptRecycle::Pop()
	{
		...
		else
		{
			...
			if(buff == nullptr) buff = new PostAcceptRecycle(common::E_CT_Accept);
		}
	...
	PostRecvRecycle* PostRecvRecycle::Pop()
	{
		...
		{
			buff = new PostRecvRecycle(common::E_CT_Recv);
		}
		else
		{
			__recvs.try_pop(buff);
			if (buff == nullptr) buff = new PostRecvRecycle(common::E_CT_Recv);
		}
	...
	PostSendRecycle* PostSendRecycle::Pop()
	{
		...
		else
		{
			...
			if (buff == nullptr) buff = new PostSendRecycle(common::E_CT_Send);
		}
