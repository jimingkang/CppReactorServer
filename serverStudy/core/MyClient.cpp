...
#ifdef ____Win32
	unsigned long ul = 1;
	int32_t errorcode = ioctlsocket(socketfd, FIONBIO, (unsigned long*)& ul);
...
{
#ifdef ____Win32
	//03、初始化Windows Sockets DLL
	WSADATA  wsData;
	int32_t errorcode = WSAStartup(MAKEWORD(2, 2), &wsData);
	if (errorcode != 0)
	{
		WSACleanup();
		return -1;
...
bool tcp::MyClient::ConnectServer()
{
...
#ifdef ____Win32
	addr.sin_addr.S_un.S_addr = inet_addr(m_data.ip);
#else
	...
	if (value < 0)
	{
		...
		{
			m_data.state = common::E_CSS_Connect;
			if (m_Notify_Accept != NULL) m_Notify_Accept(this, 2);
			return true;
		}
#else
...
{
	if (m_data.state == common::E_CSS_Free) return;
	...
	InitSocket();
	if (m_Notify_Disconnect != NULL) this->m_Notify_Disconnect(this, errcode);
}
